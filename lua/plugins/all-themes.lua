-- Every colorscheme an Omarchy theme might select, cloned but not loaded, so
-- `omarchy theme set` can switch to any of them without a network round trip.
-- Skipped off Omarchy, where none of these would ever be used.
if not require("config.omarchy").enabled() then
    return {}
end
